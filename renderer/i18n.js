...
const I18N = {
  zh: {
    ...
    tokenPh: '粘贴后点保存',
    btnTokenSave: '保存', btnTokenClear: '清除',
    tokenVerifying: '正在校验令牌…',
    tokenNone: '未设置：GitHub 未登录接口按出口 IP 限 60 次/小时，公司网络是整个办公室共用这个额度。',
    tokenSaved: (h) => `已保存（${h}），额度 5000 次/小时。`,
    tokenFromEnv: (k, h) => `由环境变量 ${k} 提供（${h}），它的优先级更高，这里存的不会生效。`,
    setTheme: '主题', themeSystem: '跟随系统', themeLight: '浅色', themeDark: '深色',
    setUpdateChannel: '更新通道', channelAll: '含预发布版', channelLatest: '仅稳定版', prereleaseTag: '预发布版',
    ...
    btnUpgrade: '升级', pluginUpToDate: '已是最新',
    pluginCheckingUpdates: '正在检查插件更新…', pluginUpdatesNone: '所有插件已是最新',
    toastUpgraded: (n) => `已升级 ${n}，重启 DSH 生效`, toastUpgradeFailed: '升级失败，详见日志',
    // update modal
    ...
    detailFailGhWhy: (why) => `获取详情失败：${why}`,
  ...
  en: {
    ...
    tokenPh: 'Paste, then Save',
    btnTokenSave: 'Save', btnTokenClear: 'Clear',
    tokenVerifying: 'Verifying token…',
    tokenNone: 'Not set: the unauthenticated GitHub API allows 60 requests/hour per egress IP, shared by everyone behind the same corporate NAT.',
    tokenSaved: (h) => `Saved (${h}) — quota is now 5000/hour.`,
    tokenFromEnv: (k, h) => `Provided by ${k} (${h}). It takes precedence, so anything saved here has no effect.`,
    setTheme: 'Theme', themeSystem: 'System', themeLight: 'Light', themeDark: 'Dark',
    setUpdateChannel: 'Update channel', channelAll: 'Include prereleases', channelLatest: 'Stable only', prereleaseTag: 'prerelease',
    ...
    btnUpgrade: 'Upgrade', pluginUpToDate: 'Up to date',
    pluginCheckingUpdates: 'Checking plugin updates…', pluginUpdatesNone: 'All plugins are up to date',
    toastUpgraded: (n) => `Upgraded ${n}; restart DSH to apply`, toastUpgradeFailed: 'Upgrade failed, see logs',
    ...
    detailFailGhWhy: (why) => `Failed to load details: ${why}`,
